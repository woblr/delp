...
impl IdStorageFormat {
    ...
    pub fn best_for(ids: &[u32]) -> Self {
        if ids.is_empty() { return Self::None; }
        if is_contiguous(ids) { return Self::None; }
...
fn encode_compressed_list(ids: &[u32]) -> Vec<u8> {
    ...
    for &id in ids.iter().skip(1) {
        ...
        writer.write_bits(delta as u32, 16);
        prev = id;
    }
    writer.finish()
}
...
fn decode_compressed_list(buf: &[u8], nb_ids: usize, first_src_id: u32) -> Result<(Vec<u32>, usize)> {
    let mut ids  = Vec::with_capacity(nb_ids);
    ids.push(first_src_id);
    if nb_ids == 1 { return Ok((ids, 0)); }

    let needed = (nb_ids - 1) * 2; // 16 bits per delta
    if buf.len() < needed {
        return Err(DelpError::MalformedEncodingVector { reason: "CompressedList buffer too short" });
    }

    let mut reader = BitReader::new(buf);
    let mut prev   = first_src_id;
    for _ in 1..nb_ids {
        let delta = reader.read_bits(16) as u32;
        prev += delta;
        ids.push(prev);
    }
...
fn decode_uncompressed_edges(buf: &[u8], nb_ids: usize, first_src_id: u32) -> Result<(Vec<u32>, usize)> {
    // Each block = 2 × u32 (start_delta, end_delta)
    let mut ids = Vec::with_capacity(nb_ids);
    let mut off = 0usize;
    while ids.len() < nb_ids {
        if buf.len() < off + 8 {
            return Err(DelpError::MalformedEncodingVector { reason: "UncompressedEdges truncated" });
        }
...
fn decode_compressed_edges(buf: &[u8], nb_ids: usize, first_src_id: u32) -> Result<(Vec<u32>, usize)> {
    let mut ids    = Vec::with_capacity(nb_ids);
    let mut reader = BitReader::new(buf);
    let mut abs    = 0u32;
    while ids.len() < nb_ids {
        if reader.remaining_bits() < 32 {
            return Err(DelpError::MalformedEncodingVector { reason: "CompressedEdges truncated" });
        }
        let d_start = reader.read_bits(16) as u32;
        let d_len   = reader.read_bits(16) as u32;
        let start   = first_src_id + abs + d_start;
        abs         = start - first_src_id + d_len;
        for id in start..start + d_len { ids.push(id); }
    }
    ids.truncate(nb_ids);
...
impl BitWriter {
    fn new() -> Self { Self { buf: Vec::new(), cur_byte: 0, bit_pos: 0 } }
...
impl<'a> BitReader<'a> {
    fn new(buf: &'a [u8]) -> Self { Self { buf, byte_i: 0, bit_pos: 0 } }

    fn read_bits(&mut self, nbits: u8) -> u32 {
        let mut val = 0u32;
        for _ in 0..nbits {
            if self.byte_i >= self.buf.len() { break; }
    ...
    fn bytes_consumed(&self) -> usize {
        if self.bit_pos > 0 { self.byte_i + 1 } else { self.byte_i }
    }
