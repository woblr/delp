use super::common::{CciLength, CommonHeader, PacketType};
...
impl FeedbackPacket {
    ...
    pub fn serialise(&self) -> Vec<u8> {
        ...
        out.resize(out.len() + (sack_padded - sack_bytes), 0);
        out
    }
    ...
    pub fn parse(buf: &[u8]) -> Result<Self> {
        ...
        let tsi = if header.has_tsi() {
            let t = u32::from_be_bytes(
                buf[CommonHeader::SIZE..CommonHeader::SIZE + 4]
                    .try_into()
                    .unwrap(),
            );
            Some(t)
