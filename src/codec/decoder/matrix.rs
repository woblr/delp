...
use crate::gf::simd::ops::{mul_acc_gf2_4, mul_acc_gf2_8, mul_scale_gf2_4, mul_scale_gf2_8};
...
impl DenseCoefs {
    ...
    fn subtract_scaled(&mut self, other: &DenseCoefs, factor: u8, field: Field) {
        if other.coefs.is_empty() || factor == 0 {
            return;
        }
...
impl MatrixRow {
    ...
    fn normalize(&mut self, field: Field) -> Result<()> {
        let pivot_c = self
            .coefs
            .pivot_coef()
            .ok_or(DelpError::MalformedEncodingVector {
                reason: "zero pivot",
            })?;
        if pivot_c == 1 {
            return Ok(());
...
fn gf_inv(v: u8, field: Field) -> Result<u8> {
    ...
    if v == 0 {
        return Err(DelpError::DivisionByZero);
    }
    Ok(match field {
...
impl DecodingMatrix {
    ...
    pub fn add_coded(
        ...
        let pivot_id = row
            ...
            .pivot_id()
            .ok_or(DelpError::MalformedEncodingVector {
                reason: "no pivot after elimination",
            })?;
        ...
        loop {
            ...
            let solved: Vec<SourceSymbolId> = self
                .rows
                .iter()
    ...
    fn back_substitute(&mut self, new_pivot: SourceSymbolId, new_row: &MatrixRow) -> Result<()> {
        ...
        for pid in self.rows.keys().copied().collect::<Vec<_>>() {
            ...
            self.rows
                .get_mut(&pid)
                .unwrap()
                .subtract_scaled(new_row, c, self.field);
        }
        Ok(())
    }
...
mod tests {
    ...
    fn round_trip_with_loss(
        ...
        for (src_id, orig) in sources.iter().enumerate() {
            ...
            let recovered = known.get(&(src_id as u32)).unwrap_or_else(|| {
                panic!("source {src_id} not recovered (loss={pct}%, seed={seed})")
            });
            assert_eq!(
                recovered.as_ref(),
                orig.as_slice(),
    ...
    fn round_trip_2_erasures_guaranteed_recovery() {
        ...
        // All 4 coded packets
        let coded: Vec<(u32, Vec<u8>)> = (1..=4u32)
            .map(|cid| {
        ...
        assert_eq!(
            known.get(&3).map(|b| b.as_ref()),
            Some(sources[3].as_slice()),
            "source 3 not recovered"
        );
        assert_eq!(
            known.get(&5).map(|b| b.as_ref()),
            Some(sources[5].as_slice()),
