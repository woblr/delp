...
use delp::gf::simd::ops::{mul_acc_gf2_4, mul_acc_gf2_8, mul_scale_gf2_8};
...
fn bench_mul_acc_gf2_8(c: &mut Criterion) {
    ...
    for size in [64usize, 256, 1024, 4096, 16384, 65536] {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", size), &size, |b, _| {
            b.iter(|| {
                mul_acc_gf2_8_reference(black_box(&mut dst), black_box(&src), black_box(0x7F))
            });
        });
    }
    group.finish();
...
fn bench_mul_acc_gf2_4(c: &mut Criterion) {
    ...
    for size in [64usize, 256, 1024, 4096, 16384] {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", size), &size, |b, _| {
            b.iter(|| {
                mul_acc_gf2_4_reference(black_box(&mut dst), black_box(&src), black_box(0x37))
            });
        });
    }
    group.finish();
